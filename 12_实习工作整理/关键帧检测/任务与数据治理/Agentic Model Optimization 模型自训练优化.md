...
#### 2.5 Workflow、Ralph Loop、Agent 与 Skill 的职责边界

Agentic Model Optimization 不是把训练命令简单串成一个 workflow。四个组件承担不同职责：

| 组件 | 回答的问题 | 典型职责 | 不应承担的职责 |
| --- | --- | --- | --- |
| Workflow / Pipeline | “怎么稳定执行一次实验？” | 数据准备、训练、推理、评测、产物落盘 | 判断下一轮该改数据还是改超参数 |
| Ralph Loop / Orchestrator | “什么时候继续、停止或回滚？” | 循环触发、任务调度、重试、状态恢复、`LOCAL_DONE` / `LOCAL_NOTDONE` 判定 | 直接理解所有 bad case 或自由编写训练命令 |
| Optimization Agent | “下一轮最值得做什么？” | 诊断指标、提出假设、选择策略、估计成本和风险 | 绕过评测门禁、无限扩大搜索空间 |
| Skill | “某一类操作如何可靠完成？” | 封装 pipeline、数据策略、超参数、评测等原子能力 | 自行改变业务目标或准出标准 |
...
Optimization Agent：
  在每轮之间读取结果，决定下一轮的实验假设和动作。

Skill：
  将动作转成有输入输出、可审计、可复用的执行能力。
```

因此，普通 workflow 的行为是预先固定的：

```text
train -> infer -> eval -> exit
```

Agentic Loop 的行为是条件化的：

```text
train -> infer -> eval
  -> 诊断是数据、规则、Prompt 还是训练配置问题
  -> 选择一个受约束的新策略
  -> train -> infer -> eval
  -> 达标则结束，否则继续或回滚
```
...
#### 6.5 受约束自动搜索，而不是无限自动科研

Agent 的“自主”应理解为在人工预先定义的可行动作集合中选择和组合策略，而不是自行定义业务真值、任意修改代码或无限试验。
...
| 策略类型 | 允许 Agent 搜索的例子 | 必须限制的边界 |
| --- | --- | --- |
| 超参数 | learning rate、batch size、warmup、epoch、采样配比 | 最小/最大值、单轮变更幅度、最大试验次数 |
| 数据策略 | hard negative 比例、replay 比例、课程学习阶段 | 训练集来源、最大新增比例、人工复查要求 |
| Prompt 策略 | 证据优先级、反例、输出 schema | 不能改变完成态真值或绕开格式约束 |
| 训练范式 | SFT、CoT-SFT、GRPO、OPD 的准入选择 | 必须满足对应数据、verifier 或 Teacher 条件 |
...
如果 Agent 不能用这些字段解释“为什么做这一轮实验”，它执行的只是不可控的 trial-and-error，而不是可靠的模型优化。
