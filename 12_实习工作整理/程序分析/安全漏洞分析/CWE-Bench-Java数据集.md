...
*   **弥补现有数据集的不足：** 许多现有数据集要么是合成的（不够真实），要么缺乏完整的漏洞信息（如修复提交、CWE ID），要么项目难以编译（[静态分析](<../静态分析基础/静态分析.md>)工具通常需要项目能成功构建）。
...
*   **可编译性 (Compilable):** 每个项目都确保可以成功编译，这是静态分析工具（如[CodeQL](<../分析工具/CodeQL.md>)）进行数据流图提取和分析的关键前提。
...
总而言之，CWE-Bench-Java 是一个经过精心策划和严格验证的、专注于真实Java项目安全漏洞的数据集。它的出现对于推动[程序分析](<../静态分析基础/程序分析.md>)（特别是LLM辅助的静态分析）领域的研究具有重要价值，因为它提供了一个更可靠和更具挑战性的评估标准。

## 面试应对

### CWE-Bench-Java 是什么？

回答思路：先定位为 Java 安全漏洞检测基准，再说明它评估什么能力。

回答模板：

CWE-Bench-Java 是面向 Java 项目的安全漏洞检测评测数据集，用来评估工具或模型发现真实 CWE 类型漏洞的能力。它关注的是漏洞样本是否真实、项目是否可编译、元数据是否完整，以及检测结果能否和 ground truth 对齐。相比只用合成小样例，它更适合衡量工具在真实 Java 工程中的漏洞识别效果。

### 这类漏洞评测数据集应该关注哪些质量维度？

回答思路：从真实性、可复现性、标注质量和评测协议回答。

回答模板：

我会关注四个维度。第一是真实性，样本最好来自真实项目和真实漏洞修复，而不是过度简化的玩具代码。第二是可复现性，项目依赖、构建方式和漏洞版本要清楚。第三是标注质量，要明确 CWE 类型、漏洞位置、修复提交和验证依据。第四是评测协议，要定义清楚什么算命中，如何统计误报、漏报、Top-K 和定位粒度。

### 用 CWE-Bench-Java 评估 LLM 有什么风险？

回答思路：强调数据泄漏、定位口径和解释可信度。

回答模板：

评估 LLM 时最大的风险是数据泄漏，因为公开漏洞、修复提交和代码片段可能已经出现在预训练数据里。其次是评测口径：模型只说“这里可能有 SQL 注入”和准确定位到文件、行号、数据流路径不是一回事。还有些模型解释听起来合理，但没有真实数据流证据。严格评测需要去重、时间切分、隐藏测试集，并结合可执行验证或人工审查。
