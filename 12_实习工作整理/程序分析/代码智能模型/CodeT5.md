...
想象一下，当程序员阅读代码 `binarySearch(...)` 时，仅仅从这个函数名就能立刻理解其核心功能是“[二分查找](<../../../10_计算机基础知识/数据结构与算法/二分查找.md>)”。CodeT5 的核心洞察就在于，代码中的**标识符 (Identifiers)**——即变量名、函数名、类名等——蕴含了极其丰富的语义信息。传统模型常常将这些标识符与其他代码词元（如 `if`, `for`, `{`, `}`）一视同仁，从而浪费了这些宝贵信息。
...
回答思路：点明它是 2021 年基于 T5 的统一 Encoder-Decoder 代码模型，最大特点是“标识符感知”。

回答模板：

CodeT5 是 Salesforce 在 2021 年提出的、基于 T5 的统一 Encoder-Decoder 代码模型，既能做理解也能做生成。它把所有任务都统一成“文本到文本”的形式，比如分类就直接生成标签、生成任务就输出代码。它最核心的洞察是代码里的标识符——变量名、函数名、类名——携带了大量语义，传统模型往往把它们和 if、for 这类普通 token 一视同仁，浪费了信息。所以 CodeT5 专门设计了标识符感知的预训练，让模型学会区分并利用这些关键 token，是 CodeT5+ 的前身。

### CodeT5 的核心机制是什么？

回答思路：围绕 T5 的掩码跨度去噪，加上标识符标记、掩码标识符预测、双模态双向生成几个预训练任务。

回答模板：

CodeT5 基于 T5 架构，预训练把标准的掩码跨度预测和几个标识符感知任务交替进行。掩码跨度预测就是 T5 经典的去噪，遮住连续片段再还原，学局部语法和上下文。标识符标记是个序列标注任务，让编码器判断每个 token 是不是标识符，等于给代码做语法高亮。掩码标识符预测更难，它只遮标识符、并且把同一个标识符的所有出现替换成同一个掩码符（相当于代码混淆），逼模型靠整体逻辑推断变量名而不是靠局部语法猜。此外还有双模态双向生成，用成对的代码-注释同时做“代码到注释”和“注释到代码”，加强代码和自然语言的对齐。

### CodeT5 的优势和限制是什么？

回答思路：从统一架构兼顾理解与生成、标识符感知带来的语义提升，以及规模较小的局限来权衡。

回答模板：

CodeT5 的优势是 Encoder-Decoder 架构天然兼顾理解和生成，在 CodeXGLUE 基准的十几个子任务上都很强：代码生成显著超过 CodeGPT 和当时的 SOTA PLBART，代码摘要超过 CodeBERT、GraphCodeBERT，代码翻译和代码修复也表现突出，甚至在漏洞检测、克隆检测这些 Encoder-only 传统强项上也不落下风。它的标识符感知预训练是关键差异点，让它比只套用 NLP 方法的模型更懂代码语义。局限主要是按现在标准它规模偏小，复杂推理和长上下文能力有限，这也是后续 CodeT5+ 用模块化设计和更大规模去改进的地方。
