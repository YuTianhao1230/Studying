...
常考传统 RAG 与 Agentic RAG 的区别、多轮检索、查询改写、证据验证、噪声控制和成本。

### 解法/回答思路

先判断问题是否需要多跳和动态检索，再说明召回、重排、证据融合、冲突处理和空证据策略。

### 易错点

把 Agentic RAG 等同于多次调用向量库，忽略权限、版本、证据绑定和无证据拒答。

### 回答模板

回答 RAG 题目时可按“问题复杂度 -> 检索规划 -> 证据校验 -> 结果生成 -> 成本与边界”说明。
## 面试应对
...
传统 RAG 通常是一次检索后直接生成答案；Agentic RAG 会先判断问题是否需要拆解，再多轮检索、改写 query、验证证据，必要时继续查。它更适合复杂问题和多跳问题，但成本、延迟和系统复杂度更高。
...
当问题需要多跳推理、首次检索信息不足、来源冲突或需要逐步缩小范围时需要多轮检索。简单事实查询通常不必使用复杂 Agentic RAG。
...
检查答案中的关键结论能否在检索片段中找到直接证据，并验证引用和结论是否一致。无法支撑的内容应标不确定或重新检索。
...
优化 query rewrite、chunk 粒度、召回源、reranker、去重和上下文压缩，同时过滤过期、低质量和与问题无关的片段。
...
它包含多轮 LLM 调用、检索、rerank、验证和上下文整理，比单轮 RAG 消耗更多 token、工具调用和延迟。
