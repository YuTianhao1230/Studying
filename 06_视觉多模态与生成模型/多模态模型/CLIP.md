...
回答思路：先一句话定位为对比学习的双塔多模态模型，再讲 batch 内对比的训练目标。

回答模板：

CLIP 是 OpenAI 2021 年提出的图文对比预训练模型，用一个图像编码器和一个文本编码器把图像和文本映射到同一个嵌入空间。训练时取一个 batch 的 N 个图文对，算出 N 个图像向量和 N 个文本向量，构成一个 N×N 的相似度矩阵，目标是让对角线上匹配的图文对相似度最高、其余 N²−N 个不匹配对相似度低，本质是一个对称的对比损失（InfoNCE），图到文和文到图各算一次交叉熵。它在从互联网收集的数亿图文对上训练，因此表示很鲁棒、迁移能力强。

### CLIP 为什么能做零样本分类？

回答思路：讲“把类别名变成文本 prompt，比相似度”的流程，突出它不需要针对任务再训练。

回答模板：

CLIP 把分类问题转成了图文匹配问题。做零样本分类时，把每个候选类别写成一句 prompt，比如“a photo of a cat”“a photo of a dog”，用文本编码器编码成向量，再用图像编码器编码待分类图像，计算图像向量和每个类别文本向量的余弦相似度，相似度最高的类别就是预测结果。因为不依赖固定的分类头，只要能用自然语言描述的类别都能识别，所以它能识别训练时没专门标注过的类别，这就是它零样本能力的来源。

### CLIP 用 ViT 还是 ResNet 做图像编码器，有什么区别？

回答思路：对比归纳偏置、数据依赖、全局建模能力和可扩展性。

回答模板：

两种都可以。ResNet 版本靠卷积和残差，归纳偏置强、对局部纹理敏感、小数据也能训，但捕捉长程依赖偏弱。ViT 版本把图像切成 patch 做自注意力，能直接建模全局关系、可扩展性更好，但归纳偏置少、更依赖大规模数据。OpenAI 在自己的超大规模数据上发现 ViT 表现和可扩展性都更好，所以后续主流 CLIP，比如 ViT-L/14，基本都用 ViT。

### CLIP 有哪些局限？在多模态系统里通常怎么用？

回答思路：先说局限（细粒度、计数、否定、非自然图），再说它作为 backbone 的地位。
