...
本文件给后续进入该工作区的 AI [Agent](<08_Agent/基础概念/Agent.md>) 使用。当前路径 `/mlx_devbox/users/yutianhao` 不是单一代码仓库，而是用户的研究工作区。最重要的前提是：不要把所有目录平铺理解为“用户的工作”。这里有主线项目、baseline/依赖仓库、数据工程项目、模型缓存和个人工具，它们的角色不同。
...
- `eval_defense.py`：防御评估入口，覆盖 JPEG、bit-depth、Gaussian blur/noise、random resize padding 等[输入预处理防御](<11_AI安全与对抗攻击/防御与鲁棒性/输入预处理防御.md>)。
...
- `hf_models/`：本地模型权重缓存，用于跑开源 MLLM 和 [CLIP](<02_大模型/模型细节/里程碑模型/CLIP.md>) proxy，包括 Qwen2-VL、LLaVA、CLIP、InstructBLIP 等。只读使用，不能当代码仓库修改。
...
编辑 `Studying/` 下的面试知识卡片时，不能只整理名词、大纲或问题列表。一篇完整卡片应让读者同时知道：

1. **知识是什么**：定义、原理、核心机制、复杂度或关键公式。
2. **怎么考**：常见选择题、计算题、代码题、场景题或面试追问形式。
3. **怎么解**：识别信号、推导步骤、代码或伪代码模板、边界条件和易错点。
4. **怎么回答**：在 `## 面试应对` 中给出回答思路和可直接复述的回答模板。

具体要求：

- `### 概述` 只总结本篇整理的知识内容，不写“为了应对某考试而整理”之类元叙述。
- 算法和计算类卡片必须包含典型题型、解题步骤、复杂度和易错点。
- 概念和工程类卡片必须包含常见追问、适用边界、方案取舍和标准回答。
- 不能用“高频问题 + 若干关键词”代替知识解析。
- 已经通过“直接回答、原理、排查流程、追问”完整展开的长文，不机械重复增加同义章节。
- 优先增量补充已有文件，不为少量内容新建目录，不设独立高频考点目录。
