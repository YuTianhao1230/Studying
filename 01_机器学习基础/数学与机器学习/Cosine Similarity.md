...
余弦相似度（Cosine Similarity）是一种衡量两个**非零向量方向相似性**的指标，通过计算向量夹角的余弦值来判断它们是否指向相近方向。它关注方向而不是长度，因此特别适合文本 embedding、向量检索、推荐[召回](<../../05_推理部署与系统/系统设计/召回粗排精排重排.md>)等需要比较语义方向的场景。
...
*   **自然语言处理 (NLP):**
    ...
    *   **词嵌入 (Word Embeddings) 相似性:** 计算 Word2Vec、GloVe、[BERT](<../../02_大模型/模型细节/里程碑模型/BERT.md>) 等模型生成的词向量之间的余弦相似度，以衡量词语之间的语义相似性（例如，"king" 和 "queen" 的向量相似度会很高）。
...
在 CV 中，图像或视频通常被转换成**高维特征向量 (Feature Vectors) 或嵌入 (Embeddings)** 来表示其内容。这些向量由各种算法提取，尤其是深度学习模型（如卷积神经网络 CNN 或 Vision [Transformer](<../../02_大模型/基础架构/Transformer.md>) [ViT](<../../02_大模型/模型细节/里程碑模型/ViT.md>)）。
...
2.  **对特征向量幅度变化的鲁棒性 (Robustness to Feature Vector Magnitude Variations):**
    *   深度学习模型提取的特征向量的幅度（长度）可能受到多种因素影响，包括训练过程中的[正则化](<../深度学习基础/正则化.md>)、网络层选择、图像本身的某些属性（如清晰度、对比度）等。
...
回答思路：先一句定义——用两向量夹角的余弦衡量方向相似性、取值在 -1 到 1，再点明它只看方向不看模长、常用于文本 embedding、向量检索和推荐召回。

回答模板：

Cosine Similarity 是余弦相似度，用两个向量夹角的余弦衡量方向相似性，取值通常在 -1 到 1 之间。它关注方向而不是模长，因此常用于文本 embedding、向量检索和推荐召回中的相似度计算。

### Cosine Similarity 的核心机制是什么？

回答思路：抓住计算公式——点积除以两向量 L2 范数的乘积，说明取值随夹角从 1（同向）到 0（正交）到 -1（反向）变化，并点出向量归一化后它就等价于点积。

回答模板：

余弦相似度等于两个向量点积除以各自 L2 范数乘积。向量方向越接近，值越接近 1；正交时接近 0；方向相反时接近 -1。实际使用前通常要做归一化，这样余弦相似度可以简化为归一化向量的点积。

### Cosine Similarity 有哪些使用场景和注意事项？

回答思路：一面讲它适合比较语义向量的方向，一面点出两个局限——会丢掉有业务含义的模长信息，且 embedding 没训练好时余弦高也不代表语义真的相近。

回答模板：

它适合比较语义向量方向，但不适合所有距离度量场景。如果向量模长本身有业务含义，余弦会忽略这部分信息；如果 embedding 没有训练好，余弦高也不代表语义一定正确。
