...
### 为什么将像素值从 [0, 255] 缩放到 [0, 1]，可以减少计算量并加速神经网络收敛？ 是什么？

回答思路：先给定义，再说明它在 机器学习基础 中解决的问题，最后补一句工程边界。

回答模板：

为什么将像素值从 [0, 255] 缩放到 [0, 1]，可以减少计算量并加速神经网络收敛？ 的核心含义是：归一化（Normalization）是将数据按比例缩放到特定范围（如 [0, 1] 或 [-1, 1]）的数据预处理技术，旨在消除不同特征之间的量纲差异，提升模型性能和数据可比性。 它出现的背景是为了解决具体任务中的效果、效率、稳定性或工程复杂度问题。 机制上，LayerNorm对**单个样本**在某一层所有神经元（或特征）的输出进行归一化，而非像BatchNorm那样对一个批次（Batch）内的样本做归一化。 使用时还要注意边界：它并不是通用万能方案，必须结合数据质量、成本和实际指标验证收益。

### 为什么将像素值从 [0, 255] 缩放到 [0, 1]，可以减少计算量并加速神经网络收敛？ 解决什么问题？
...
### 为什么将像素值从 [0, 255] 缩放到 [0, 1]，可以减少计算量并加速神经网络收敛？ 的核心机制是什么？

回答思路：拆关键模块和执行流程，说明它如何影响效果、效率或稳定性。

回答模板：

Normalization 的机制要从输入、处理过程和输出结果三层理解。**核心方法** 1. 面试里我会进一步说明关键步骤如何连接，以及这些步骤为什么会影响效果、效率或稳定性；同时也要说明机制成立的前提，比如数据质量、参数配置和计算成本。

### 为什么将像素值从 [0, 255] 缩放到 [0, 1]，可以减少计算量并加速神经网络收敛？ 有哪些优劣和限制？

回答思路：优点从效果、效率、稳定性讲；限制从数据、成本、复杂度和适用边界讲。

回答模板：

为什么将像素值从 [0, 255] 缩放到 [0, 1]，可以减少计算量并加速神经网络收敛？ 的价值要和限制一起看。优势上，归一化（Normalization）是将数据按比例缩放到特定范围（如 [0, 1] 或 [-1, 1]）的数据预处理技术，旨在消除不同特征之间的量纲差异，提升模型性能和数据可比性。 但它也可能带来额外成本，例如数据依赖、实现复杂度、调参难度、稳定性风险或线上维护成本。真实项目里不能只看理论收益，而要通过 ablation、分桶评测、bad case 分析和成本指标确认净收益。

### 项目中如何验证 为什么将像素值从 [0, 255] 缩放到 [0, 1]，可以减少计算量并加速神经网络收敛？ 有效？
