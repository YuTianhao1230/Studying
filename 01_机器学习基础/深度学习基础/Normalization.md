...
- **[可视化](<../../07_Python与工程/常用库/可视化.md>)**：将不同量纲的数据映射到相同范围，便于直观对比。
...
3. **兼容初始化与[正则化](<正则化.md>)**：与权重初始化、批标准化等技术协同，提升训练稳定性。
4. **硬件友好性**：适配浮点运算优化，降低数值异常风险。

虽然单次计算量变化不大，但通过改善训练动态（如减少迭代次数、允许更大学习率），显著降低了整体训练成本。

### layer norm是什么
**Layer Normalization（层归一化）** 是一种用于神经网络中的标准化技术，旨在通过调整每一层神经元的输出分布来加速模型训练、提升稳定性和泛化能力。它主要应用于**序列模型**（如[Transformer](<../../02_大模型/基础架构/Transformer.md>)、RNN）和**小批量训练**场景，与Batch Normalization（批归一化）形成互补。
...
### **应用场景**
1. **Transformer模型**  
   - 每个编码器/解码器层的输出后接LayerNorm（如[BERT](<../../02_大模型/模型细节/里程碑模型/BERT.md>)、[GPT](<../../02_大模型/模型细节/里程碑模型/GPT.md>)）。
   - 示例代码（PyTorch）：
     ```python
...
2. **循环神经网络（RNN）**  
   - 在[LSTM](<../../02_大模型/基础架构/LSTM.md>)或GRU的隐藏状态后添加LayerNorm，缓解梯度消失/爆炸。
...
回答思路：一句定义——调整输入或中间激活的尺度分布让优化更稳定，再列举输入标准化、BatchNorm、LayerNorm、[RMSNorm](<../../02_大模型/基础架构/RMSNorm.md>) 等常见形式。

回答模板：

Normalization 是归一化或标准化技术，用来调整输入或中间激活的尺度分布，让优化更稳定。常见形式包括输入标准化、BatchNorm、LayerNorm、RMSNorm 等。
...
它通过控制均值、方差或均方根尺度，减少不同特征或不同层之间的分布差异，从而改善梯度传播和训练稳定性。不同归一化方法的统计维度不同，比如 BatchNorm 依赖 batch 统计，LayerNorm 在单个样本 hidden 维度上归一化。
...
选择 Normalization 要看任务和模型结构。CNN 中 BatchNorm 常见，Transformer 中 LayerNorm/RMSNorm 更常见。小 batch 或分布变化大时，BatchNorm 可能不稳定；大模型中还要考虑归一化位置、数值精度和训练稳定性。
