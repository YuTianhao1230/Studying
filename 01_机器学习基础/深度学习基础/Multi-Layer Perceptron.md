...
• **MLP通过隐藏层和激活函数（如[ReLU](<../../03_训练优化与对齐/参数/常见激活函数.md>)、Sigmoid）**，将输入数据映射到高维空间，从而拟合复杂决策边界。
...
• **[Transformer](<../../02_大模型/基础架构/Transformer.md>)中的FFN（前馈网络）**：  
  每个Transformer层中的MLP负责对注意力机制的输出进行非线性变换（如[GPT](<../../02_大模型/模型细节/里程碑模型/GPT.md>)、[BERT](<../../02_大模型/模型细节/里程碑模型/BERT.md>)）。  
• **CNN分类头**：  
...
• **输入层**：接收原始数据（如向[量化](<../../05_推理部署与系统/推理工程/量化.md>)的图像、文本、表格数据）。  
...
### **9. 局限性及改进**
• **参数量大**：全连接层导致参数量随输入维度爆炸增长（可通过Dropout[正则化](<正则化.md>)缓解过拟合）。  
...
### Multi-Layer Perceptron 是什么？

回答思路：一句定义——由多层线性变换加非线性激活堆叠而成的前馈网络，再点明它是最基础的神经网络、也是 Transformer FFN 等复杂模型的基础模块。

回答模板：

MLP 是 Multi-Layer Perceptron，多层感知机，由多层线性变换和非线性激活组成。它是最基础的前馈神经网络，也是很多复杂模型里的基础模块。

### Multi-Layer Perceptron 的核心机制是什么？

回答思路：抓住“线性映射+激活引入非线性、多层堆叠逼近复杂函数”这条主线，并举 Transformer 里 FFN 就是逐 token 的 MLP 说明其作用。

回答模板：

MLP 每层先做线性映射，再通过激活函数引入非线性，多层堆叠后可以拟合复杂函数。在 Transformer 中，FFN 本质上就是作用在每个 token 上的 MLP，用来提升表示的非线性变换能力。

### Multi-Layer Perceptron 有哪些使用场景和注意事项？

回答思路：点出它结构简单、表达力强但不建模序列顺序、空间局部性和图结构，所以处理文本/图像/图数据时要搭配 attention、卷积或图网络。

回答模板：

MLP 结构简单、表达能力强，但不显式建模序列顺序、空间局部性或图结构。处理文本、图像或图数据时，通常会和 attention、卷积或图网络结合使用。
