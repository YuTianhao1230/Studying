...
在面试中提到 Joern ，你展示的是对“代码静态分析”领域 学术前沿 和 开源工具链 的深刻理解。如果说 CodeQL 是工业界的“老大哥”（严谨、强大但重型），那么 Joern 就是静态分析界的“特种兵”： 快速、灵活，且是 CPG（代码属性图）这一概念的“亲生父母”。 它的目标通常是利用代码的语法结构、控制流、数据流或上下文语义，提升代码理解、检索、生成、漏洞发现或程序分析能力。
...
在面试中提到 Joern ，你展示的是对“代码静态分析”领域 学术前沿 和 开源工具链 的深刻理解。如果说 CodeQL 是工业界的“老大哥”（严谨、强大但重型），那么 Joern 就是静态分析界的“特种兵”： 快速、灵活，且是 CPG（代码属性图）这一概念的“亲生父母”。 关键是说明它如何利用代码结构，而不是把代码完全当成普通自然语言文本。
...
代码层： 原始代码（文本）。结构层： 使用 Joern 转化为 CPG 。序列化层： 将 CPG 转化为 DOT 或 JSON 。模型层： Encoder-only： 结合文本和 Joern 路径做高精度的漏洞检测。 实际系统通常会把静态分析、动态测试、人工审查和模型能力结合起来，以降低单一方法的盲区。
