...
在面试中提到 CodeQL ，你不仅是在聊一个工具，而是在聊 “如何将代码语义化、结构化” 的核心工程能力。CodeQL 是目前大模型（LLM）在安全领域落地时，最强有力的“脚手架”。 它的目标通常是利用代码的语法结构、控制流、数据流或上下文语义，提升代码理解、检索、生成、漏洞发现或程序分析能力。
...
在面试中，讲清楚这个流程能体现你的专业度： 解析与建模（Extraction）： CodeQL 会“编译”你的代码。它不仅仅是看文本，还会捕捉代码的 AST（抽象语法树） 、 CFG（控制流图） 和 数据流关系 ，并将这些信息存入私有的数据库。创建数据库（Database）： 这个数据库包含了程序的所有语义信息（变量在哪里定义、哪里被修改、函数如何跳转）。运行查询（Executing Queries）： 开发者编写 .ql 文件。 关键是说明它如何利用代码结构，而不是把代码完全当成普通自然语言文本。
...
“我将 CodeQL 视为模型感知现实世界的‘显微镜’。在我的方案中，我不只是把代码当成字符串丢给 LLM，而是利用 CodeQL 将代码转化为包含 AST、CFG、DFG 的多模态图结构（CPG） 。这种‘强语义工具（CodeQL）+ 强推理模型（LLM）’的组合，解决了成熟软件中 0-day 漏洞难以被发现的痛点。特别是在 数据集自动构建 和 修复方案的闭环验证 上，CodeQL 是不可或缺的基础设施。 实际系统通常会把静态分析、动态测试、人工审查和模型能力结合起来，以降低单一方法的盲区。
