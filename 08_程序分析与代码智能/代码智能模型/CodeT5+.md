...
[CodeT5](<CodeT5.md>)+ 是由 Salesforce AI Research 团队在 CodeT5 的成功基础上，于2023年推出的一个**功能更强大、架构更灵活、训练更高效**的开源代码大型语言模型家族。它的核心目标是克服现有代码模型的局限性，提供一个能够“一站式”解决几乎所有代码理解和生成任务的通用框架。
...
它主要解决了现有代码模型的两大痛点：
1.  **架构僵化**：Encoder-only模型不擅长生成，Decoder-only模型不擅长理解，而传统的[Encoder-Decoder](<../../02_大模型/基础架构/Decoder-only vs Encoder-Decoder.md>)模型在代码补全等任务上又不够高效。
...
*   **第一阶段：在单模态代码数据上学习** (Unimodal Pretraining)
    ...
    *   **任务**：混合进行多种任务，让模型从不同角度学习代码：
        1.  **Span Denoising (跨度去噪)**：[T5](<../../02_大模型/模型细节/里程碑模型/T5.md>)的经典任务，随机遮盖代码中的连续片段，让模型恢复它们。—— 学习代码的**局部上下文**。
...
*   **第二阶段：在双模态代码-文本数据上学习** (Bimodal Pretraining)
    ...
    *   **任务**：进一步引入跨模态任务，学习代码和自然语言的对齐：
        1.  **Text-Code Contrastive Learning (文本-代码对比学习)**：拉近匹配的代码-注释对的向量表示，推远不匹配的。—— 学习**高级语义相似性**。
        2.  **Text-Code Matching (文本-代码匹配)**：输入一对代码-注释，让模型判断它们是否匹配。—— 学习**细粒度的对齐**。
...
*   **浅编码器 + 深解码器架构**：他们发现生成任务通常比理解任务更复杂，所以解码器需要更强大。
...
CodeT5+ 在**超过20个**代码相关的基准测试上进行了全面评估，并在发布时的多个任务上取得了有竞争力或领先的结果。这里的 SOTA 结论只对应当时的基准和对比范围，不代表当前所有代码模型中的排名。

*   **代码生成 (HumanEval)**：
    *   **InstructCodeT5+ 16B** 版本在 HumanEval 上的 pass@1 分数达到了 **35.0%**，在当时的开源模型中取得了新的 SOTA 成绩，甚至超过了 OpenAI 的闭源模型 `code-cushman-001`。
*   **代码补全**：
    *   在 Decoder-only 模式下，其性能远超 [UniXcoder](<UniXcoder.md>) 和 CodeGen-multi 等模型，证明了其架构的灵活性和混合预训练的有效性。
*   **代码理解 (文本-代码检索)**：
...
**CodeT5+ 是代码语言模型发展中的一个重要里程碑，它标志着模型设计从单一架构、单一目标向模块化、多任务、高效率的转变。**
...
回答思路：点明它是 2023 年 CodeT5 的升级、模块化可组合的开源代码大模型家族，强调架构灵活和高效扩展。

回答模板：

CodeT5+ 是 Salesforce 在 2023 年基于 CodeT5 推出的开源代码大模型家族。它不是一个固定模型，而是一堆预训练好的编码器和解码器组件，可以像搭积木一样按任务组合，激活 Encoder-only、Decoder-only 或 Encoder-Decoder 模式。它主要解决 CodeT5 那代模型的两个痛点：一是架构僵化，理解型不擅长生成、生成型不擅长理解、传统 Encoder-Decoder 在补全上又不够高效；二是预训练任务单一，能力有偏向。它的目标是用一个模块化框架“一站式”覆盖几乎所有代码理解和生成任务。

### CodeT5+ 的核心机制是什么？

回答思路：围绕两阶段混合预训练、模块化组合三种模式，以及“冻结重用大模型”的高效扩展来讲。

回答模板：

CodeT5+ 的关键是两阶段混合预训练加模块化架构。第一阶段在海量纯代码上混合做跨度去噪、序列到序列因果语言建模、以及纯解码器的因果语言建模，分别学局部上下文、长距离依赖和生成能力；第二阶段在代码-注释对上加入文本-代码对比学习、匹配和双向生成，学跨模态对齐。预训练后组件可以按任务灵活激活成 Encoder-only、Decoder-only 或 Encoder-Decoder 三种模式。它还有个很聪明的扩展策略：用“浅编码器加深解码器”，直接拿现成的 CodeGen 之类模型初始化，训练时冻结巨大的解码器，只训小编码器和连接两者的少量交叉注意力层，用很少的算力就能得到很强的大模型。此外还有用指令数据微调出的 InstructCodeT5+。

### CodeT5+ 的优势和限制是什么？

回答思路：从模块化带来的性能与效率平衡、开源 SOTA 成绩，以及组件初始化依赖现有模型等角度权衡。

回答模板：

CodeT5+ 的优势是模块化加混合预训练带来了性能和效率的平衡，能一套框架适配多种任务。它在 20 多个基准上很强：InstructCodeT5+ 16B 在 HumanEval 上 pass@1 达到 35%，是当时开源 SOTA，甚至超过 OpenAI 的 code-cushman-001；Decoder-only 模式的补全远超 UniXcoder、CodeGen-multi；检索任务上比 UniXcoder 平均 MRR 高约 3.2 个点；甚至 770M 版本在 MathQA、GSM8K 上超过大得多的 LLaMA 65B。它还能天然当作检索增强生成系统，用编码器检索、解码器生成。相对局限是它靠冻结重用现成大模型来扩展，能力上限受限于所选基座，而且模块化组合和分阶段训练本身对工程和调度要求更高。
