...
AST、CFG、DFG 是[程序分析](<程序分析.md>)里三种由浅入深的代码表示：AST（抽象语法树）刻画代码的语法结构，CFG（控制流图）在其上刻画语句之间的执行流转，DFG（数据流图）再进一步刻画数据在程序中如何定义与传播，三者层层递进，支撑起大多数[静态分析](<静态分析.md>)。
...
2.  **结构过于庞大和复杂**：
    ...
    *   **对模型不友好**：这种巨大的、层级很深的的树结构对于深度学习模型（尤其是 [Transformer](<../../02_大模型/基础架构/Transformer.md>)）来说，处理起来计算成本高，且容易丢失长距离依赖信息。
...
1.  **清晰表示执行流程**：
    ...
    *   **应用**：这使它成为分析程序动态行为的利器，如：
        ...
        *   **程序优化**：编译器利用 CFG 来进行循环优化、指令[重排](<../../05_推理部署与系统/系统设计/召回粗排精排重排.md>)等。
...
2.  **无法表示数据依赖**：
    *   由于缺乏数据信息，CFG 无法用于需要追踪变量来源和使用的分析，如变量未初始化检查、[污点分析](<污点分析.md>)等。
...
1.  **深刻揭示程序语义**：
    ...
    *   **应用**：在需要深度语义理解的任务中表现出色：
        ...
        *   **漏洞检测 (污点分析)**：追踪一个受污染的输入数据（如用户输入）是否流动到了一个危险的操作（如执行SQL查询），这是 [GraphCodeBERT](<../代码智能模型/GraphCodeBERT.md>) 等模型能用于漏洞分析的基础。
...
**最终结论**：这三者没有绝对的优劣，而是**互补**的。一个好的代码分析系统或代码语言模型，理想情况下应该能综合利用这三种信息。**GraphCodeBERT** 的成功就在于它在 [CodeBERT](<../代码智能模型/CodeBERT.md>) (基于文本序列) 的基础上，创新地引入了 DFG，从而在语义理解上迈出了一大步。而像 **[UniXcoder](<../代码智能模型/UniXcoder.md>)** 这样的后续模型，则试图将 AST 和 DFG 的信息更全面地融合起来，以期获得更强大的综合能力。
