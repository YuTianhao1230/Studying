...
回答思路：先说明它属于代码模型、静态分析还是安全评测，再讲输入输出。

回答模板：

好的，我们来详细深入地讲解一下 污点分析 (Taint Analysis) 。这是一种在程序分析，尤其是安全领域，极其重要的技术。 它的目标通常是利用代码的语法结构、控制流、数据流或上下文语义，提升代码理解、检索、生成、漏洞发现或程序分析能力。

### 污点分析 的核心机制是什么？

回答思路：围绕代码表示、图结构、规则传播或预训练目标回答。

回答模板：

想象一下，你正在处理一种有毒的化学物质。污染源 (Source) ：化学物质的瓶子。这是“脏”东西的来源。传播 (Propagation) ：你用手接触了化学物质，你的手变“脏”了。 关键是说明它如何利用代码结构，而不是把代码完全当成普通自然语言文本。

### 污点分析 的优势和限制是什么？

回答思路：从覆盖率、误报漏报、语义理解和工程成本回答。

回答模板：

举个简单的 PHP 例子来说明： 总结： 敏感函数（Sink）是漏洞爆发的地点。污点分析的核心任务就是证明： 是否存在一条从 Source 到 Sink 的路径，且该路径上没有任何有效的过滤措施。 实际系统通常会把静态分析、动态测试、人工审查和模型能力结合起来，以降低单一方法的盲区。
