...
好的，我们来详细深入地讲解一下**污点分析 (Taint Analysis)**。这是一种在[程序分析](<程序分析.md>)，尤其是安全领域，极其重要的技术。
...
污点分析本质上是一种**[数据流分析](<数据流分析.md>) (Data-Flow Analysis)** 的特例。其静态实现步骤如下：
...
*   **字符串复制/拼接：**
    ...
    *   `sprintf(buffer, ...)`: [格式化输出](<../../07_Python与工程/Python语法/格式化输出.md>)长度不可控。
*   **内存操作：**
...
在静态应用程序安全测试（SAST）工具（如 Coverity, Fortify, Checkmarx 或开源的 [CodeQL](<../分析工具/CodeQL.md>)）中，**敏感函数列表（Sink List）是核心知识库**。
...
回答思路：先定义 Source、Sink、Sanitizer，再说明它用于发现不可信数据流向危险操作。

回答模板：

污点分析是一种程序安全分析方法，用来追踪不可信输入是否会经过程序传播到危险操作。这里不可信输入叫 Source，比如用户请求参数；危险操作叫 Sink，比如 SQL 执行、命令执行、文件写入；中间的过滤或校验叫 Sanitizer。污点分析要判断是否存在一条从 Source 到 Sink 且没有有效 Sanitizer 的数据流路径。

### 污点分析的核心机制是什么？

回答思路：围绕数据流传播规则、控制流路径和过滤规则回答。

回答模板：

污点分析通常先构建程序的 AST、CFG 或数据流图，然后根据规则标记 source 变量为 tainted。随着赋值、函数调用、对象字段传递和字符串拼接，污点会沿数据依赖继续传播。如果 tainted 数据到达 sink，且路径上没有被可信 sanitizer 清理，就报告潜在漏洞。核心难点在于跨函数、跨文件、动态特性和框架语义建模。

### 污点分析有什么优缺点？

回答思路：优点讲可解释和覆盖，限制讲误报漏报和规则成本。

回答模板：

污点分析的优点是结果可解释，可以给出从输入到危险调用的路径，适合发现 SQL 注入、命令注入、XSS 等数据流型漏洞。限制是规则和框架建模成本高，source/sink/sanitizer 定义不准会带来误报或漏报；动态语言、反射、复杂别名和隐式流也会增加分析难度。因此工程上常把污点分析和规则扫描、单元测试、动态验证、LLM 辅助审计结合使用。
