...
回答思路：先说明它切断当前张量和计算图的梯度关系，再强调它通常共享底层数据。

回答模板：

`.detach()` 是 PyTorch Tensor 的一个方法，用来返回一个新的张量视图，这个新张量和原张量共享数据，但不再连接到原来的 autograd 计算图。也就是说，它的 `requires_grad` 通常是 `False`，没有原来的 `grad_fn`，后续基于它的计算不会把梯度回传到原来的计算路径。它常用于记录 loss、转 NumPy、构造 target 或阻断某个分支的梯度。

### `.detach()` 和 `torch.no_grad()` 有什么区别？

回答思路：从作用对象和作用范围区分。

回答模板：

`.detach()` 作用在某一个张量上，表示从这个点开始把该张量从已有计算图里分离出来。`torch.no_grad()` 是上下文管理器，作用在一段代码上，表示这段代码里的操作都不构建梯度图。简单说，`.detach()` 是对已有张量做截断，`no_grad()` 是让后续一段前向过程不被 autograd 记录。推理阶段通常用 `no_grad()` 或 `inference_mode()`，而训练中只想阻断某个分支梯度时通常用 `.detach()`。

### 为什么记录 loss 时常用 `.item()` 或 `.detach()`？

回答思路：说明不应把日志变量继续挂在计算图上，避免额外显存占用。

回答模板：

训练时如果只是为了打印或保存 loss 数值，就不需要保留它的计算图。直接把带梯度历史的 tensor 存进列表，可能导致计算图引用一直存在，造成显存持续增长。常见做法是用 `loss.item()` 得到 Python 标量，或者先 `loss.detach()` 再转 CPU、转 NumPy。核心原则是：日志、指标和[可视化](<../常用库/可视化.md>)数据不应该参与反向传播。

### `.detach()` 有什么风险？

回答思路：强调它会阻断梯度，错误使用会让模型参数得不到更新。

回答模板：

`.detach()` 最大的风险是误切断梯度。如果在需要训练的主路径上对中间激活做了 detach，后面的 loss 就无法把梯度传回前面的参数，模型可能看起来正常跑，但关键模块没有被更新。另外，detach 后的张量通常和原张量共享 storage，如果再做原地修改，也可能引入难查的问题。工程上我会用 `requires_grad`、`grad_fn`、参数梯度是否为 None，以及小规模反向测试来确认梯度路径符合预期。
