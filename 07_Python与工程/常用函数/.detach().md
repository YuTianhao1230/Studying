...
训练时如果只是为了打印或保存 loss 数值，就不需要保留它的计算图。直接把带梯度历史的 tensor 存进列表，可能导致计算图引用一直存在，造成显存持续增长。常见做法是用 `loss.item()` 得到 Python 标量，或者先 `loss.detach()` 再转 CPU、转 NumPy。核心原则是：日志、指标和可视化数据不应该参与反向传播。
