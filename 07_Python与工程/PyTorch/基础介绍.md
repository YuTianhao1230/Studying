...
好的，非常乐意为你详细介绍这四个 PyTorch 的核心支柱。理解了它们，你就掌握了构建和训练几乎所有神经网络模型的关键。 它的价值在于让代码表达更清楚，或者让张量、数据、控制流、日志和调试过程更可靠。
...
这是日常使用中最频繁的部分。在训练循环中，优化器总是遵循一个固定的三步流程： optimizer.zero grad() : 清空旧的梯度 。如果不清零，梯度会累加。loss.backward() : 计算新的梯度 。 使用时要确认输入类型、返回值、是否修改原对象、是否影响梯度，以及在循环或大数据场景下是否有性能开销。
...
现在我们把这四个组件串联成一个完整的（伪）训练流程： 通过这个流程，你应该能清晰地看到这四个核心组件是如何协同工作，完成一个神经网络的训练任务的。它们各司其职，共同构成了 PyTorch 强大而灵活的生态系统。 写工程代码时，最重要的是用小例子验证行为，并通过断言、日志或单元测试固定边界条件。
