...
并查集（Disjoint Set Union, DSU）是一种高效处理集合「合并」与「查询」的数据结构，支持两个核心操作——Union 把两个元素所在的集合合并、Find 查某元素属于哪个集合，配合路径压缩和按秩合并可做到近似 O(1)，常用于连通性判断、朋友圈计数和环检测。下面是一个带路径压缩的典型实现：
...
*   **查询**：你想知道王五和赵六是不是在同一个朋友圈里。你只需要看看他们是否有一个**共同的老大**（或者说，他们是否属于同一个圈子）。
...
回答思路：先抽象成动态连通性问题，再说明 Union 和 Find 两个操作。

回答模板：

并查集主要解决动态维护若干元素所属集合的问题，典型场景是判断两个点是否连通、统计连通分量数量、检测无向图是否成环，以及 Kruskal 最小生成树。它把每个集合表示成一棵树，每个元素记录父节点，集合的代表元素是根节点。核心操作只有两个：`find(x)` 找到 x 所在集合的根，`union(x, y)` 把两个元素所在集合合并。

### 并查集为什么快？

回答思路：重点讲路径压缩和按秩或按大小合并。

回答模板：

并查集快是因为它用两类优化把树压得很扁。路径压缩是在 `find` 的过程中，把沿途节点直接挂到根节点下面，之后再查这些节点会很快。按秩合并或按大小合并是在 `union` 时把矮树挂到高树上，或者把小集合挂到大集合上，避免树退化成链表。两者结合后，单次操作的均摊复杂度接近 O(1)，严格来说是 O(alpha(n))。

### 做题时怎么判断该用并查集？

回答思路：从关键词和图结构特征识别，不要只背模板。

回答模板：

如果题目里反复出现“合并集合”“判断是否属于同一组”“连通分量数量”“朋友圈/省份数量”“冗余连接”“等式可满足性”这类表述，我会优先考虑并查集。它适合处理边逐渐加入、集合关系不断合并的问题；如果问题要求最短路、拓扑顺序或路径权重，单纯并查集通常不够，需要换成 BFS、DFS、Dijkstra 或拓扑排序。

### 并查集实现有哪些坑？

回答思路：检查初始化、路径压缩返回值、重复合并、集合计数和索引边界。

回答模板：

实现并查集时，首先要把 `parent[i]` 初始化为 `i`，如果要统计集合大小或连通分量，还要维护 `size` 或 `count`。`find` 里路径压缩要返回根节点，不能只改一层父节点就结束。`union` 前要先找两个根，只有根不同才合并，并且只有发生真实合并时才减少连通分量数量。做矩阵题时还要注意只遍历上三角，避免重复处理同一条无向边。
