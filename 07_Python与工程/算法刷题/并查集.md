...
并查集（Disjoint Set Union, DSU） 是一种非常优雅且高效的数据结构，专门用来处理“集合合并”与“元素查询”的问题。 它通常用于把暴力枚举、重复查询或复杂状态搜索，转化成更清晰的数据结构、状态转移或剪枝过程。
...
想象一下你在组织一个大型派对，想知道现场有多少个互不相识的“朋友圈”。初始状态 ：每个人都只认识自己，所以每个人都是一个独立的“朋友圈”。（ n 个人， n 个朋友圈） 新关系 ：你得知“张三”和“李四”是朋友。于是，他们的两个朋友圈就合并成了一个更大的朋友圈。 正确性来自状态定义完整、转移规则不漏不重，以及边界条件处理清楚。
...
是什么 ：并查集是一种维护集合关系的数据结构。做什么 ：支持高效的 Find (查找代表) 和 Union (合并集合) 操作。为什么快 ： 路径压缩 + 按秩/大小合并 这两大优化，使得其平均操作时间复杂度接近 O(1) （严格来说是 O(α(n))，其中 α 是反阿克曼函数，增长极其缓慢，在宇宙尺度内都小于5）。用在哪 ： 计算图的连通分量数（省份数量问题）。 复杂度分析要说明主要开销来自哪里；实现时重点检查初始化、边界、重复状态、递归深度和是否需要剪枝或压缩。
