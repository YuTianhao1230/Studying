...
在 Python 中，[哈希表](<../../12_计算机基础知识/数据结构与算法/哈希表.md>)的内置实现就是**字典（`dict`）**。
...
类似于[列表推导式](<列表推导式.md>)，可以快速地从一个可迭代对象创建字典。
...
回答思路：点明它是 Python 内置的哈希表，用 key-value 存储、增删查平均 O(1)，key 需可哈希。

回答模板：

字典是 Python 的哈希映射结构，用 key-value 形式存储数据，平均查找、插入和删除复杂度接近 O(1)。

### 字典 适合什么场景？

回答思路：抓住索引、缓存、计数、分组这些用途，并提醒 key 要可哈希、插入有序不等于业务排序。

回答模板：

它适合做索引、缓存、计数、分组和配置管理。使用时要注意 key 必须可哈希，以及遍历顺序虽然在新版 Python 中保持插入顺序，但不应滥用为业务排序逻辑。

### 字典 常见坑是什么？

回答思路：抓住遍历时改大小会报错、key 必须可哈希、缺失键取值用 get/defaultdict 这几个真实坑。

回答模板：

字典最常见的坑是遍历时增删 key：直接在 `for k in d` 里改变字典大小会抛 `RuntimeError: dictionary changed size during iteration`，要先 `list(d.keys())` 拿一份快照再改，或者另存要删的 key。第二个坑是 key 必须可哈希，拿 list、dict、set 这种可变对象当 key 会 `TypeError: unhashable type`。第三个是用 `d[k]` 取不存在的键会 KeyError，不确定存不存在时应该用 `d.get(k, default)` 或 defaultdict。另外别把“插入有序”当成“自动排序”，需要排序还是得显式 sorted。
