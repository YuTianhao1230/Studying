...
lambda 是一种用来创建 匿名函数 （anonymous function）的关键字。匿名 ：意味着这个函数没有正式的名字（不像用 def 定义的函数）。简洁 ：它通常只有一行，用于实现一些简单的功能。 它的价值在于让代码表达更清楚，或者让张量、数据、控制流、日志和调试过程更可靠。
...
lambda 的威力并不在于像上面那样给它命名后使用，而是在于 将它作为参数传递给其他高阶函数 （Higher-order Functions），比如 sorted() , map() , filter() 等。 使用时要确认输入类型、返回值、是否修改原对象、是否影响梯度，以及在循环或大数据场景下是否有性能开销。
...
只能有一个表达式 ：这是它最大的限制。你不能在 lambda 中写多行逻辑。但你可以使用三元运算符来实现简单的条件判断。保持简洁 ： lambda 的设计初衷就是为了简洁。 写工程代码时，最重要的是用小例子验证行为，并通过断言、日志或单元测试固定边界条件。
