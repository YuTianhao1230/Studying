...
⭐⭐⭐⭐⭐ 第一梯队：必须掌握，日常天天用 print() : 打印输出，调试和显示结果的必备工具。 它的价值在于让代码表达更清楚，或者让张量、数据、控制流、日志和调试过程更可靠。
...
这些函数作用于字符串、列表、元组等可迭代对象上。len(s) : 返回长度。sorted(iterable) : 返回一个排序后的 新 列表。enumerate(iterable) : 返回 (索引, 值) 对。 使用时要确认输入类型、返回值、是否修改原对象、是否影响梯度，以及在循环或大数据场景下是否有性能开销。
...
⭐⭐⭐⭐⭐ 第一梯队：必须掌握，日常天天用 print() : 打印输出，调试和显示结果的必备工具。 写工程代码时，最重要的是用小例子验证行为，并通过断言、日志或单元测试固定边界条件。
