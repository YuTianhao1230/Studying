...
Counter 第一个坑是访问不存在的 key 返回 0 而不是抛 KeyError，用起来方便，但 key 拼错时也不会报错，容易埋 bug；而且单纯读 `c['x']` 不会把这个 key 插进去，但 `c['x'] += 1` 会。第二个坑是减法：`c1 - c2` 会自动丢掉结果里小于等于 0 的计数，想保留负数得用 `c1.subtract(c2)`。还有 `most_common()` 在计数相同时是按插入顺序排的，不是按 key 排序，不能依赖它做字典序。
