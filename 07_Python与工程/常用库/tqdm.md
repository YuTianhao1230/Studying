...
回答思路：点明它把可迭代对象包一层就显示进度条、速率和 ETA，让长循环不再像卡死。

回答模板：

`tqdm` 是 Python 常用进度条库，用来显示循环或迭代任务的进度、速度和预计剩余时间。

### tqdm 适合什么场景？

回答思路：抓住训练/数据处理这类耗时循环适用，但别在内层高频刷新、别让多进程日志和进度条互相污染。

回答模板：

它适合训练、数据处理、批量推理等耗时循环。注意不要在内层高频循环过度刷新，也不要让多进程日志和进度条互相污染。

### tqdm 常见坑是什么？

回答思路：抓住内层高频刷新拖慢、多进程只在 rank0 显示、手动/generator 要传 total、Jupyter 用 notebook 版这几个坑。

回答模板：

我踩过的 tqdm 坑主要有几个：一是在内层高频循环里每次迭代都刷新，进度条本身就成了瓶颈，这时要调大 `mininterval`/`miniters` 或干脆只在外层套 tqdm；二是多进程或分布式训练里每个进程都打一条进度条、再混上 `print` 日志，终端会被刷得乱七八糟，我一般只在 rank 0 显示，并用 `position` 参数把多条进度条错开；三是手动 `while` 循环或包 generator 时不传 `total`，就没有百分比和 ETA，要显式给 `total`；四是在 Jupyter 里得用 `from tqdm.notebook import tqdm`，否则进度条会一行行往下刷屏。
