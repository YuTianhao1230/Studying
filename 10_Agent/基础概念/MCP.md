...
1. MCP 解决了什么问题？
   ...
   - 回答模板：MCP 解决的是 Agent 接入外部工具、资源和 Prompt 的标准化问题。没有 MCP 时，每个工具都要单独适配；有 MCP 后，Agent 可以用统一协议发现和调用外部能力。
2. MCP 的 Tools、Resources、Prompts 分别是什么？
   - 回答思路：先给一句话定义，再说明它解决什么问题，最后补一个工程例子或常见风险。
   - 回答模板：Tools 是可执行动作，比如查数据库；Resources 是可读取上下文，比如文件或文档；Prompts 是可复用提示模板。三者分别对应行动、信息和指令。
3. MCP Client 和 MCP Server 的职责是什么？
   - 回答思路：先给一句话定义，再说明它解决什么问题，最后补一个工程例子或常见风险。
   - 回答模板：MCP Client 在 Agent 侧负责发现能力和发起调用；MCP Server 负责暴露工具、资源、Prompt 并执行具体逻辑。Client 不直接实现业务能力，Server 才真正连接外部系统。
4. MCP 工具很多时会有什么问题？
   - 回答思路：先定义问题，再说明核心机制、适用边界、风险和验证方式。
   - 回答模板：工具太多会让工具描述占满上下文，模型选择困难，误调用概率上升，延迟也会增加。需要按场景路由、分组、权限控制和 progressive disclosure。
5. MCP 和 Skill 如何配合？
   - 回答思路：先定义问题，再说明核心机制、适用边界、风险和验证方式。
   - 回答模板：MCP 提供可调用的工具接口，Skill 提供何时调用、如何调用、注意事项和工作流。可以理解为 MCP 是工具插座，Skill 是使用说明和最佳实践。
