...
我会先分别定义相关概念，再从目标、输入数据、核心机制、工程成本和适用场景对比。围绕 Skill，关键是说明它解决的是哪类问题，以及相比相近方案的取舍：有的方案更简单稳定，有的方案探索能力更强但成本更高。最后要落到工程选择标准，而不是只列名词。
...
description 决定模型什么时候加载这个 Skill。写得太宽会误触发、污染上下文；写得太窄会漏触发。好的 description 要明确能力边界和典型场景。
...
通常 `SKILL.md` 放触发条件、核心流程和必须遵守的规则；`references/` 放长文档；`scripts/` 放可复用脚本；`assets/` 放模板和资源。目标是按需加载。
...
高频、短小、必须遵守的流程放 `SKILL.md`；较长、低频、按场景读取的资料放 references，以降低上下文成本。
...
用真实任务集评估触发准确率、任务成功率、步骤遵循率、错误率和上下文成本。还要看它是否减少重复错误，而不是只看有没有被加载。
