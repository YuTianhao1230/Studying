...
[Skill](<../基础概念/Skill.md>) 不是普通代码文档，而是给 [Agent](<../基础概念/Agent.md>) 注入的“可调用上下文”。写 Skill 的目标不是把所有知识写全，而是在正确时机给模型提供它原本容易做错、漏做或不稳定执行的关键约束。
...
回答思路：点明它是给 Agent 按需注入的可调用上下文，专治模型会稳定做错/漏做的那类任务，而非从零推理。
...
回答思路：抓住 description 写触发意图、SKILL.md 只放高信号规则、复杂资料下沉 references 按需读、先写 eval 正反例。
...
回答思路：抓住 gotchas 飞轮（从失败沉淀反例）、description 不轻易改要有 eval 支撑、新增 Skill 做边界回归防误触发。
