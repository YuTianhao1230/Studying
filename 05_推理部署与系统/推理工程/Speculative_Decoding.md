...
Speculative Decoding 的基本原理 是 Speculative Decoding 里的核心概念，主要解决系统在能力、效率、稳定性或可控性上的问题。面试中要说明它的定义、适用场景、限制，以及在真实工程中如何验证它有效。
...
小模型生成候选 token 的成本低，大模型可以一次验证多个候选。只要接受率足够高，就能减少大模型串行 decode 次数。
...
Speculative_Decoding 的核心含义是：Speculative Decoding 是一种大模型推理加速方法：用小模型先快速草拟多个 token，再由大模型并行验证，从而减少大模型逐 token 解码次数。 面试回答时我会先给定义，再说明它解决的背景问题，接着讲核心机制和适用场景，最后补充限制与工程风险。这样回答比只背一句概念更完整。

### 它和 KV Cache、Batching、量化分别解决什么问题？

回答思路：先定义问题，再说明核心机制、适用边界、风险和验证方式。

回答模板：

KV Cache 减少历史重复计算，Batching 提高并发吞吐，量化降低显存和计算成本，Speculative Decoding 减少大模型 decode 步数。
...
要用真实流量或压测比较接受率、TTFT、TPOT、p95/p99、吞吐、成本和质量。如果只改善平均值但恶化长尾或质量，就不适合全量。
