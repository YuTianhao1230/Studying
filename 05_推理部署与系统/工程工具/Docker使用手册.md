...
回答思路：抓住“环境一致性”这个核心，说明容器把依赖、运行时和启动方式打包，消除本地能跑、线上跑不了的差异，落到模型服务部署和实验复现场景。
...
回答思路：用“静态模板 vs 运行实例、一个镜像可起多个容器”把两者讲清，并强调容器运行态易失、重要数据要挂载到外部卷。
...
回答思路：列举镜像过大、依赖版本不固定、密钥写进镜像、端口挂载路径不清等坑，并特别点出 GPU 场景要保证 CUDA、驱动、PyTorch 与系统库版本匹配。
