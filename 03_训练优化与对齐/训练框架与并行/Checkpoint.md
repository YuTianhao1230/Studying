...
1. checkpoint 是什么？
   ...
   - 回答模板：checkpoint 是 Checkpoint 里的一个核心概念，主要用来解决系统在能力、效率、稳定性或可控性上的问题。面试中我会把它解释为一种具体机制，并补充它适合什么场景、有什么限制，以及在真实工程中如何验证它是否有效。
2. 断点续训需要保存哪些状态？
   - 回答思路：先定义问题，再说明核心机制、适用边界、风险和验证方式。
   - 回答模板：这个问题在 Checkpoint 场景下，核心答案是说明它解决什么实际工程问题，以及如何在系统里落地。完整回答需要覆盖目标、执行方式、失败风险和验证指标。
3. best checkpoint 和 last checkpoint 有什么区别？
   - 回答思路：先分别定义两个概念，再从目标、机制、适用场景和工程取舍四个角度对比。
   - 回答模板：先分别定义两个对象，再比较目标、机制、适用场景和风险。围绕 Checkpoint，关键是说明它们解决的问题不同。
4. LoRA checkpoint 如何用于推理？
   - 回答思路：先定义问题，再说明核心机制、适用边界、风险和验证方式。
   - 回答模板：这个问题在 Checkpoint 场景下，核心答案是说明它解决什么实际工程问题，以及如何在系统里落地。完整回答需要覆盖目标、执行方式、失败风险和验证指标。
5. 如何做 checkpoint 版本管理？
   - 回答思路：按“目标定义 -> 关键步骤 -> 风险控制 -> 指标验证”的顺序回答，避免只讲抽象原则。
   - 回答模板：我会把 做 checkpoint 版本管理 拆成几个步骤处理：先明确目标和输入输出，再设计关键模块或策略，然后加入失败处理和权限/质量约束，最后用离线指标、线上指标或回归测试验证效果。在 Checkpoint 场景里，回答重点是能落到真实工程流程，而不是只列方法名。
