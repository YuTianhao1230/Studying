...
1. JAX 和 PyTorch 的核心区别是什么？
   ...
   - 回答模板：JAX 和 PyTorch 的核心区别 的区别要看目标和控制方式。JAX 更偏一种机制或能力，PyTorch 的核心区别 更偏另一类抽象或系统边界。在 JAX与XLA 场景里，面试时要说明二者不是互斥关系，而是经常组合使用：稳定部分交给更确定的机制，开放或动态部分交给更灵活的机制。
2. `jit`、`vmap`、`pmap` 分别解决什么问题？
   - 回答思路：先定义问题，再说明核心机制、适用边界、风险和验证方式。
   - 回答模板：这个问题在 JAX与XLA 场景下，核心答案是说明它解决什么实际工程问题，以及如何在系统里落地。完整回答需要覆盖目标、执行方式、失败风险和验证指标。
3. 为什么 JAX 更强调函数式编程？
   - 回答思路：先指出背后的核心约束，再解释收益，最后补充如果不这样做会带来的风险。
   - 回答模板：函数式代码输入输出明确、隐式状态少，更适合 XLA trace、jit 编译、自动向量化和多设备并行。显式 PRNGKey 也让随机性更可控。
4. XLA 能带来哪些优化？
   - 回答思路：先定义问题，再说明核心机制、适用边界、风险和验证方式。
   - 回答模板：这个问题在 JAX与XLA 场景下，核心答案是说明它解决什么实际工程问题，以及如何在系统里落地。完整回答需要覆盖目标、执行方式、失败风险和验证指标。
5. 为什么输入 shape 变化会导致重新编译？
   - 回答思路：先指出背后的核心约束，再解释收益，最后补充如果不这样做会带来的风险。
   - 回答模板：XLA 会针对具体 shape 生成优化后的执行图；shape 变化后原图的内存规划和 kernel 可能不再适用，所以需要重新 trace 和 compile。
