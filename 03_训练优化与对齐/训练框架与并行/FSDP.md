...
我会先分别定义相关概念，再从目标、输入数据、核心机制、工程成本和适用场景对比。围绕 FSDP，关键是说明它解决的是哪类问题，以及相比相近方案的取舍：有的方案更简单稳定，有的方案探索能力更强但成本更高。最后要落到工程选择标准，而不是只列名词。
...
FSDP 的核心含义是：FSDP，Fully Sharded Data Parallel，是 PyTorch 提供的全参数切分数据并行方案，把模型参数、梯度和优化器状态切分到不同 GPU 上，降低单卡显存压力。 面试回答时我会先给定义，再说明它解决的背景问题，接着讲核心机制和适用场景，最后补充限制与工程风险。这样回答比只背一句概念更完整。
...
主要来自每层计算前的参数 all-gather 和反向后的 reduce-scatter。wrap 粒度、网络带宽、层大小和 activation checkpoint 都会影响通信成本。
...
当 DDP 单卡显存放不下模型、梯度或优化器状态时适合 FSDP，尤其是大模型训练和微调。若模型较小且通信成为瓶颈，DDP 可能更简单。
