...
> **核心主线：**基础能力 → 预训练直觉 → 垂类适配选择 → 数据工程 → [SFT](<后训练与对齐/SFT 监督微调.md>) → 偏好优化 → GRPO/RLVR → 蒸馏 → 多模态视频 → 评测与故障排查 → 部署。

> **最重要的判断：**训练垂类模型时，不是把 SFT、[DPO](<后训练与对齐/DPO 直接偏好优化.md>)、[GRPO](<后训练与对齐/GRPO 组相对策略优化.md>) 全部堆上去就会变强。每一步都应该对应一个明确问题：数据不够先补数据，格式不稳先 SFT，偏好不对再 DPO，指标可验证再 GRPO/RLVR，线上用不动再做推理优化。
...
| Continued Pretraining | 在已有模型上继续做语言模型训练。 | 补领域语料、术语、风格和知识分布。 | 不是所有知识问题都要继续预训练，很多可以用 [RAG](<../02_大模型/应用与问题/RAG.md>)。 |
| [SFT](<后训练与对齐/SFT 监督微调.md>) | 用输入和标准答案做监督微调。 | 让模型学会任务格式、领域表达和基本解法。 | SFT 不能自动解决偏好排序和复杂探索。 |
| [DPO](<后训练与对齐/DPO 直接偏好优化.md>) | 用 chosen/rejected 偏好对训练模型。 | 让模型更偏向高质量答案。 | DPO 通常不是替代 SFT，而是接在 SFT 后面。 |
| [RLHF](<后训练与对齐/RLHF 基于人类反馈的强化学习.md>) | 用人类偏好训练奖励模型，再用强化学习优化。 | 适合复杂开放式偏好和安全对齐。 | 工程复杂，reward model 可能学偏。 |
| [RLVR](<后训练与对齐/RLVR 可验证奖励强化学习.md>) | 用可验证奖励做强化学习。 | 适合数学、代码、格式、检索、视频时间定位等可自动打分任务。 | reward 写错会让模型钻规则漏洞。 |
| [GRPO](<后训练与对齐/GRPO 组相对策略优化.md>) | 用同一问题的多条采样回答做组内相对优化。 | 常用于推理和可验证任务后训练。 | GRPO 是否有效主要取决于 reward、采样和稳定性。 |
...
| SFT | 模型是否会按任务格式稳定输出。 | 监督微调、[LoRA](<后训练与对齐/LoRA 低秩适配.md>) 或全参训练。 | 稳定 baseline。 |
| [DPO](<后训练与对齐/DPO 直接偏好优化.md>) 或偏好优化 | SFT 后是否主要是答案质量和偏好不稳定。 | 构造 chosen/rejected，训练 DPO。 | 偏好更好的模型。 |
| [GRPO/RLVR](<后训练与对齐/GRPO 组相对策略优化.md>) | 任务是否能自动打分，reward 是否和指标一致。 | 采样、打分、KL 约束、策略优化。 | 可验证指标进一步提升。 |
| 评测和故障排查 | 提升是否真实，失败样本属于哪类。 | 固定 test set、错误分析、日志分析。 | 下一轮数据或训练方案。 |
| 部署 | 模型是否能稳定、快速、便宜地服务。 | [vLLM](<../05_推理部署与系统/推理工程/vLLM.md>)、[量化](<../05_推理部署与系统/推理工程/量化.md>)、batch、缓存、推理配置固定。 | 可上线或可复现实验服务。 |
...
|-|-|-|-|-|-|
| [Attention](<../02_大模型/基础架构/Self-Attention.md>) Is All You Need | RNN/CNN 难以并行建模长距离依赖。 | 提出 [Transformer](<../02_大模型/基础架构/Transformer.md>) 和 self-attention。 | 长序列二次复杂度带来成本问题。 | 理解 LLM 架构基础。 | [NeurIPS 2017](https://papers.nips.cc/paper_files/paper/2017/hash/3f5ee243547dee91fbd053c1c4a845aa-Abstract.html) |
...
| [ZeRO](<训练框架与并行/ZeRO.md>) | 大模型训练显存放不下。 | 切分优化器状态、梯度和参数。 | 通信和配置复杂。 | 理解分布式训练。 | [SC 2020](https://dl.acm.org/doi/10.5555/3433701.3433727) |
...
| Context Length | 模型一次能看到的最大上下文。 | 长视频、长文档和 [Agent](<../10_Agent/基础概念/Agent.md>) 轨迹依赖长上下文。 |
...
| [Qwen](<../02_大模型/模型细节/Qwen千问架构.md>) Technical Report | 中文、多语言、代码和工具能力需要更强开放底座。 | 构建 Qwen 系列基础和对话模型。 | 不同版本差异大，训练前要确认具体模型能力。 | 和中文、多模态、Agent 训练直接相关。 | [论文](https://arxiv.org/abs/2309.16609) |
...
> **判断原则：**如果问题是“模型不知道”，考虑继续预训练或 RAG；如果问题是“模型知道但不会按要求做”，先 [SFT](<后训练与对齐/SFT 监督微调.md>)；如果问题是“模型会做但选择不够好”，再做 [DPO](<后训练与对齐/DPO 直接偏好优化.md>)；如果问题是“有明确自动指标还能继续优化”，再做 [GRPO/RLVR](<后训练与对齐/GRPO 组相对策略优化.md>)。
...
| 方法 | 核心思想 | 优点 | 风险 |
|-|-|-|-|
| [Reward Model](<后训练与对齐/Reward Model 与 Grader 奖励模型与评分器.md>) + [PPO](<后训练与对齐/PPO 近端策略优化.md>) | 先训练奖励模型，再用 PPO 优化策略模型。 | 表达能力强，适合复杂偏好。 | 流程复杂，reward model 可能被利用。 |
...
| 学生 on-policy rollout | 用 [OPD](<后训练与对齐/On-Policy Distillation 在线策略蒸馏.md>) 在学生自己的错误前缀上接受教师软监督。 | 需要教师 logits 或等价信号，推理成本高。 | 对齐 tokenizer、模板和输入，过滤教师错误并做能力回归。 |
...
- [ ] 如果学生的部署错误与教师离线轨迹差异明显，再考虑 [On-Policy Distillation](<后训练与对齐/On-Policy Distillation 在线策略蒸馏.md>)，不要把它当作普通 response SFT 的同义词。
...
| 论文 / 工作 | 解决了什么问题 | 提出了什么方法 | 还有什么不足 | 为什么读 | 链接 |
|-|-|-|-|-|-|
| [CLIP](<../06_视觉多模态与生成模型/多模态模型/CLIP.md>) | 视觉和语言缺少大规模对齐。 | 图文对比学习。 | 对视频时序和细粒度定位不足。 | 理解多模态对齐基础。 | [ICML 2021](https://proceedings.mlr.press/v139/radford21a.html) |
| [Flamingo](<../02_大模型/模型细节/里程碑模型/Flamingo.md>) | 语言模型难以处理图像和视频上下文。 | Perceiver Resampler 和 cross-attention。 | 闭源且训练成本高。 | 理解早期多模态大模型。 | [NeurIPS 2022](https://proceedings.neurips.cc/paper_files/paper/2022/hash/960a172bc7fbf0177ccccbb411a7d800-Abstract-Conference.html) |
| BLIP-2 | 端到端训练视觉语言模型成本高。 | Q-Former 连接冻结视觉模型和语言模型。 | 复杂视频推理仍有限。 | 理解低成本视觉语言对齐。 | [ICML 2023](https://proceedings.mlr.press/v202/li23q.html) |
| [LLaVA](<../02_大模型/模型细节/里程碑模型/LLaVA.md>) | 开源多模态指令模型不足。 | 视觉指令数据连接视觉编码器和 LLM。 | 视频时序和精确定位不足。 | 理解多模态 SFT。 | [NeurIPS 2023](https://proceedings.neurips.cc/paper_files/paper/2023/hash/6dcf277ea32ce3288914faf369fe6de0-Abstract-Datasets_and_Benchmarks.html) |
...
| 训练能跑但评测 OOM | generation [KV cache](<../05_推理部署与系统/推理工程/KV_Cache与Prefill_Decode.md>)、max_new_tokens、eval batch 过大。 | 降低 eval batch 和 max_new_tokens。 | 高 |
...
| optimizer step OOM | optimizer state、ZeRO/FSDP 配置。 | AdamW 的一阶/二阶矩状态占显存，全参训练尤其明显。 | 使用 ZeRO-2/3、[FSDP](<训练框架与并行/FSDP.md>)、optimizer offload，或改 LoRA。 | optimizer step 不再 OOM。 |
