...
LoRA 的核心含义是：**LoRA**，全称 **Low-Rank Adaptation（低秩适配）**，是一种 **参数高效微调（PEFT）** 技术。 它出现的背景是为了解决具体任务中的效果、效率、稳定性或工程复杂度问题。在 LoRA 出现之前，微调大模型（如 GPT-3）的主流方式是**全量微调（Full Fine-Tuning）**。这种方式存在几个致命的缺点： 机制上，LoRA 的天才之处在于其背后的一个关键假设（来自微软研究员的原始论文）：**大语言模型在适应新任务时，其参数矩阵的“变化量”是低秩的。** 使用时还要注意边界：它由华盛顿大学的 Tim Dettmers 等人在 2023 年提出。简单来说，**QLoRA 是 LoRA 的“极致省钱升级版”**，它让开发者能在消费级显卡（如 RTX 3090/4090）上微调原本需要昂贵 A100 显卡才能跑动的大模型。
...
LoRA 的机制要从输入、处理过程和输出结果三层理解。什么是 LoRA？ 面试里我会进一步说明关键步骤如何连接，以及这些步骤为什么会影响效果、效率或稳定性；同时也要说明机制成立的前提，比如数据质量、参数配置和计算成本。
...
LoRA 的价值要和限制一起看。优势上，它由华盛顿大学的 Tim Dettmers 等人在 2023 年提出。简单来说，**QLoRA 是 LoRA 的“极致省钱升级版”**，它让开发者能在消费级显卡（如 RTX 3090/4090）上微调原本需要昂贵 A100 显卡才能跑动的大模型。 但它也可能带来额外成本，例如数据依赖、实现复杂度、调参难度、稳定性风险或线上维护成本。真实项目里不能只看理论收益，而要通过 ablation、分桶评测、bad case 分析和成本指标确认净收益。
