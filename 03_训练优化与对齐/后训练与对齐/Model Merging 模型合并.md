...
Model Merging（模型合并）是在**不重新训练或只用很少额外计算**的情况下，把多个模型或多个 adapter 的能力融合到一个模型里的技术。它常用于把不同任务微调得到的模型、不同 [LoRA](<LoRA 低秩适配.md>) adapter、不同 checkpoint 或不同能力模型合并成一个统一模型。
