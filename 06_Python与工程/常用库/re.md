...
回答思路：先说明它是 Python 正则表达式库，再讲匹配、提取、替换和分割。

回答模板：

`re` 是 Python 标准库中的正则表达式模块，用来按模式处理字符串。普通字符串方法只能查固定文本，而正则可以描述一类文本模式，比如数字、邮箱、日期、日志字段。常见用途包括校验格式、提取字段、替换敏感信息和按复杂分隔符切分文本。
...
`re.match` 只从字符串开头尝试匹配，开头不符合就失败；`re.search` 会在整个字符串中找第一个匹配；`re.findall` 会返回所有不重叠匹配结果。实际使用时，如果要校验整个字符串，通常还要配合 `^`、`$` 或使用 `fullmatch`，否则可能只匹配到局部内容。

### 正则表达式有哪些常见坑？

回答思路：从贪婪匹配、转义、性能和解析器选择回答。

回答模板：

常见坑包括贪婪匹配范围过大、反斜杠转义写错、没有使用 raw string、复杂正则导致性能问题，以及用正则强行解析结构化格式。比如 JSON、HTML、XML 或 Markdown 表格，优先使用对应解析器；正则更适合格式相对稳定、边界清楚的文本模式。
