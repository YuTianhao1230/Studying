...
*   `map()`, `filter()`: 函数式编程工具。（现在更流行用[列表推导式](<列表推导式.md>)代替）
...
回答思路：点明它们是解释器免导入直接可用的通用工具，覆盖类型转换、序列操作、聚合和反射几大类。

回答模板：

Python 内置函数是解释器直接提供的通用工具，比如 `len`、`sum`、`sorted`、`zip`、`enumerate`、`any`、`all`。

### 内置函数 适合什么场景？

回答思路：抓住它们能省样板代码，但要清楚返回类型、惰性迭代和复杂度，别写成难读的一行流。

回答模板：

熟练使用内置函数能减少样板代码，但要理解返回类型、惰性迭代和复杂度，避免写出看似简洁但难以读懂的表达式。

### 内置函数 常见坑是什么？

回答思路：抓住别用内置名当变量、map/filter/zip 是一次性迭代器、max/min 对空序列抛错这三类坑。

回答模板：

内置函数的坑首先是别用它们的名字当变量名，像 `list = [...]`、`sum = 0`、`id`、`type`、`dict` 一旦被赋值就把内置函数覆盖了，后面再调用就报错。第二是返回类型：`sorted` 返回新列表，但 `map`、`filter`、`zip`、`enumerate` 返回的是一次性迭代器，忘了 `list()` 或遍历两遍就会拿到空结果。第三是边界：`max`/`min` 对空序列会抛 `ValueError`，要传 `default=` 兜底；`sum` 默认从 0 累加，不能直接对字符串求和。
