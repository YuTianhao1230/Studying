...
**你可以这样朗读它：** “对于 `iterable` 中的每一个 `item`，计算 `expression` 的值，然后把这些值收集到一个新列表中。”
...
回答思路：先给定义，再说明它把生成列表的循环逻辑压缩到表达式中。

回答模板：

列表推导式是 Python 用来快速构造列表的语法形式，基本结构是 `[expression for item in iterable]`，也可以带条件过滤。它本质上仍然是在遍历可迭代对象，只是把“遍历、计算、收集结果”写在一个表达式里。它适合简单映射和过滤场景，可以让代码更短、更符合 Python 风格。

### 列表推导式里 `if` 和 `if-else` 有什么区别？

回答思路：区分过滤条件和条件表达式的位置。

回答模板：

`[x for x in nums if x > 0]` 里的 `if` 是过滤条件，只保留满足条件的元素。`[x if x > 0 else 0 for x in nums]` 里的 `if-else` 是条件表达式，会对每个元素都产生一个结果，只是根据条件选择不同值。简单说，`for` 后面的 `if` 会减少元素数量，`for` 前面的 `if-else` 不改变元素数量。
...
如果逻辑包含多层嵌套、复杂分支、异常处理或中间变量，列表推导式会降低可读性，这时普通 for 循环更合适。另外，列表推导式会一次性生成完整列表，如果数据量很大且只需要迭代消费，可以改用生成器表达式，避免占用过多内存。
