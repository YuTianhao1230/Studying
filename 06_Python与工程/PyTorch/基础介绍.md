...
*   **[Batching](<../../05_推理部署与系统/推理工程/Batching.md>)**: 将数据打包成一小批一小批（batch）。
...
回答思路：按 Tensor、Autograd、Module、Dataset/DataLoader、Optimizer 串起来。

回答模板：

PyTorch 的核心组件可以从训练流程理解：`Tensor` 表示数据和参数，Autograd 负责自动求导，`nn.Module` 组织模型结构，`Dataset` 和 `DataLoader` 负责样本读取与 batch 构造，Optimizer 根据梯度更新参数。一次标准训练循环就是取 batch、前向计算、算 loss、反向传播、优化器更新和清空梯度。
...
PyTorch 使用动态图机制，代码执行方式接近普通 Python，调试体验好，适合研究和快速迭代。它的生态也比较完整，包括分布式训练、混合精度、数据加载、模型库和推理部署工具。对于算法工程师来说，PyTorch 的优势不是只会调 API，而是能看懂梯度、shape、显存和训练循环中的问题。

### PyTorch 训练代码常见错误有哪些？

回答思路：覆盖梯度清零、train/eval、device、shape 和 no_grad。

回答模板：

常见错误包括忘记 `optimizer.zero_grad()` 导致梯度累积，验证阶段忘记 `model.eval()` 或 `torch.no_grad()`，tensor 和模型不在同一个 device，输入输出 shape 和 loss 期望不匹配，以及误用 `.detach()` 切断梯度。排查时我会先检查 loss 是否正常下降，再看梯度是否存在、shape 是否符合预期、显存是否异常增长。
