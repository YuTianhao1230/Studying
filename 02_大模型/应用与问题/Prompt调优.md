...
4.  **使用示例 (Few-Shot Learning)**
    *   给模型一两个（或几个）输入和期望输出的例子，模型能更好地理解你的模式和要求。这对于[格式化输出](<../../07_Python与工程/Python语法/格式化输出.md>)、特定风格的写作等非常有效。
    *   例如：
        ```
...
6.  **逐步引导与分解任务 (Step-by-Step / Chain of Thought / Decomposition)**
    ...
    *   **Chain of Thought ([CoT](<CoT.md>))**：在Prompt中加入类似“让我们一步一步地思考”或展示一个思考过程的例子，可以引导模型进行更复杂的推理。
...
4.  **参数高效微调 (Parameter-Efficient Fine-Tuning, [PEFT](<../../03_训练优化与对齐/后训练与对齐/PEFT 参数高效微调.md>)) 中的软提示 (Soft Prompts)**
    ...
    *   **[LoRA](<../../03_训练优化与对齐/后训练与对齐/LoRA 低秩适配.md>) (Low-Rank Adaptation)**: 虽然不直接是“提示”调优，但它是一种SOTA的PEFT方法，通过在模型层中注入可训练的低秩矩阵来适配模型，效果显著且高效。有时与软提示方法结合或作为其替代方案。

5.  **检索增强生成 (Retrieval Augmented Generation, [RAG](<RAG.md>))**
...
7.  **基于反馈的迭代优化**
    *   **[RLHF](<../../03_训练优化与对齐/后训练与对齐/RLHF 基于人类反馈的强化学习.md>) (Reinforcement Learning from Human Feedback)**: 虽然主要用于模型预训练后的对齐调整，但其核心思想（收集人类偏好数据来优化模型输出）也可以启发Prompt的迭代。
...
7.  **思维链 (Chain-of-Thought, CoT)**
    这是提升模型在逻辑、推理、计算等复杂任务上表现的王牌技巧。**核心是让模型一步一步地思考**，先把思考过程写出来，再给出最终答案。
...
回答思路：先说明它是不改模型参数的行为引导，再讲适用边界。

回答模板：

Prompt 调优是通过设计输入指令、上下文、示例和输出约束，来引导大模型产生更符合目标的回答。它不改变模型参数，成本低、迭代快，适合原型验证、应用层优化和轻量任务适配。但它的能力上限受模型本身限制，如果模型缺少某种知识或能力，单靠 prompt 很难稳定补齐。

### 好的 prompt 通常包含什么？

回答思路：从任务、角色、上下文、约束、示例和输出格式回答。

回答模板：

一个好的 prompt 通常会清楚说明任务目标、输入背景、模型应该遵守的规则、输出格式和必要示例。对于复杂任务，我会把要求拆成可执行步骤，并明确哪些信息可以使用、哪些不能编造。对于结构化输出，我会给字段定义和反例，必要时让模型先做检索或校验。核心是减少歧义，让模型知道目标、边界和评价标准。

### Prompt 调优和微调有什么区别？

回答思路：用成本、稳定性、泛化和维护方式对比。

回答模板：

Prompt 调优是在输入层面引导模型，优点是成本低、上线快、可解释，缺点是对模型能力和上下文长度敏感，稳定性有限。微调是通过训练改变模型参数或适配层，适合固化某类任务能力、风格或格式，但成本更高，也需要数据和评测闭环。实际工程里通常先用 prompt 验证任务是否可做，若需求稳定、样本充足且 prompt 难以保证一致性，再考虑 [SFT](<../../03_训练优化与对齐/后训练与对齐/SFT 监督微调.md>) 或偏好优化。

### Prompt 调优有哪些常见风险？

回答思路：强调脆弱性、上下文污染、模板过拟合和评测不足。

回答模板：

Prompt 最大的问题是脆弱：换模型、换输入分布或上下文变长后，原来有效的提示可能失效。过长的规则也可能互相冲突，导致模型抓不住重点。还有一种风险是只在少量样例上调 prompt，看起来效果好，但在真实流量里泛化很差。因此 prompt 调优必须配合代表性测试集、bad case 回归和版本管理，不能只靠主观试几条样例。
