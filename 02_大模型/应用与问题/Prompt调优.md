...
4.  **参数高效微调 (Parameter-Efficient Fine-Tuning, PEFT) 中的软提示 (Soft Prompts)**
...
Prompt 调优是在输入层面引导模型，优点是成本低、上线快、可解释，缺点是对模型能力和上下文长度敏感，稳定性有限。微调是通过训练改变模型参数或适配层，适合固化某类任务能力、风格或格式，但成本更高，也需要数据和评测闭环。实际工程里通常先用 prompt 验证任务是否可做，若需求稳定、样本充足且 prompt 难以保证一致性，再考虑 SFT 或偏好优化。
