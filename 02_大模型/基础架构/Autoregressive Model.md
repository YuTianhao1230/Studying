...
回答思路：先用条件概率分解定义，再联系大语言模型逐 token 生成。

回答模板：

自回归模型是把序列的联合概率分解成一连串条件概率的生成模型，也就是每一步都基于已经出现的内容预测下一个元素。在大语言模型里，它表现为给定 prompt 后，从左到右逐 token 预测下一个 token，并把新生成的 token 再加入上下文继续预测。GPT 类模型就是典型的自回归语言模型。

### 自回归生成的优缺点是什么？

回答思路：优势讲灵活和生成质量，限制讲串行 decode 和误差累积。

回答模板：

自回归模型的优点是概率建模清晰，能生成任意长度序列，并且每一步都显式依赖已有上下文，因此适合文本、代码等序列生成。缺点是生成阶段天然串行，下一个 token 必须等上一个 token 生成后才能继续，所以长文本推理延迟高；同时前面生成错了，后面会继续基于错误上下文生成，容易产生误差累积。

### 自回归模型和 Masked LM 有什么区别？

回答思路：从训练目标和适用任务区分 GPT 与 BERT。

回答模板：

自回归模型通常做从左到右的 next token prediction，适合开放式生成，比如对话、写作和代码生成。Masked LM 像 BERT，会在输入中遮住部分 token，让模型利用双向上下文预测被遮住的位置，更适合理解类任务，比如分类、匹配和抽取。简单说，GPT 类自回归模型更偏生成，BERT 类 Masked LM 更偏理解。
