...
1. **文本编码**：输入文本通过[CLIP](<../../模型细节/里程碑模型/CLIP.md>)等模型转换为嵌入向量。
...
回答思路：讲前向加噪、反向去噪、训练时预测噪声这三点。

回答模板：

扩散模型分前向和反向两个过程。前向过程是逐步往数据里加高斯噪声，经过很多步后数据变成纯噪声，这个过程是固定的、无需学习。反向过程是训练一个网络，从纯噪声出发一步步去噪，最终恢复出真实数据。训练目标其实很简单：给一张图加上某个时间步 t 的噪声，让网络预测加进去的噪声是什么，用预测噪声和真实噪声的 L2 损失优化。推理时从随机噪声开始，反复调用网络去噪就能采样出新样本。相比 GAN，它训练稳定、不容易模式崩溃，缺点是要多步采样、比较慢。

### LDM 为什么要在潜空间做扩散？带来什么好处？

回答思路：抓住“像素空间太贵”这个动机，讲 autoencoder 压缩 + 潜空间扩散。

回答模板：

传统扩散直接在像素空间做，比如 512×512×3 的图，每步去噪计算量都很大，非常贵。LDM 的核心改进是先用一个预训练的自编码器把图像压缩到低维潜空间，比如 64×64×4，扩散过程全在这个潜空间里做，最后再用解码器还原成图像。因为潜空间维度低了一个量级，计算量能降十倍以上，训练和采样都快很多。同时自编码器已经把人眼不敏感的高频冗余去掉了，保留了语义信息，所以生成质量并没有明显损失。Stable Diffusion 就是这么做的。

### LDM 是怎么实现文本控制生成的？

回答思路：讲文本编码 + cross-attention 注入 U-Net。

回答模板：

靠条件控制机制，最常用的是 cross-attention。先把文本用 CLIP 这类文本编码器编码成向量，作为 key 和 value，去噪 U-Net 里的特征作为 query，通过 cross-attention 把文本信息注入到每一步去噪里，注意力权重决定文本对生成的控制强度。这样在去噪的每一步，模型都会参考文本语义来调整生成方向，最终生成和文本对齐的图像。对于类别标签这种简单条件，也可以直接拼接，但文本这种复杂条件基本都用 cross-attention。

### 扩散模型和 GAN、VAE 相比有什么取舍？

回答思路：从训练稳定性、生成质量多样性、采样速度三个维度对比。

回答模板：

主要在三个维度上权衡。训练稳定性上，扩散显式优化噪声预测的 L2 损失，最稳，GAN 是对抗博弈容易训崩，VAE 居中。生成质量和多样性上，扩散既高保真又多样、不容易模式崩溃，GAN 保真但多样性可能不足，VAE 多样但偏模糊。采样速度上 GAN 单步生成最快，扩散要多步迭代最慢，这也是扩散最大的短板，所以才有各种加速采样和蒸馏的工作。总体来说扩散是用采样速度换来了训练稳定和高质量。
