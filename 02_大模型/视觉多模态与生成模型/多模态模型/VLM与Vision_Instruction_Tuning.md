...
现在主流的 VLM（[LLaVA](<../../模型细节/里程碑模型/LLaVA.md>)、Qwen-VL、InternVL 等）几乎都遵循同一个范式：

1. **视觉编码器（Vision Encoder）**：通常是 [CLIP](<../../模型细节/里程碑模型/CLIP.md>) [ViT](<../../模型细节/里程碑模型/ViT.md>) 或 SigLIP，把图像编码成一串视觉 token（patch embedding）。用预训练好的对比学习视觉塔，是因为它的视觉表示已经和语言对齐得比较好。
2. **连接器（Projector / Connector）**：把视觉特征投影到 LLM 的词嵌入空间。最简单的是一个 MLP（LLaVA），复杂一些的用 Q-Former（BLIP-2）或 cross-attention（[Flamingo](<../../模型细节/里程碑模型/Flamingo.md>)）压缩视觉 token 数量。
