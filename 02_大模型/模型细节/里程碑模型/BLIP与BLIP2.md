...
CLIP 是双塔对比模型，主要学习全局图文对齐、检索和 zero-shot 分类，本身不直接生成文本。BLIP 用 MED 统一理解与生成，训练目标同时包含图文对比、图文匹配和语言建模，因此可以支持检索、VQA 和图像描述。BLIP 还用 CapFilt 生成并过滤 caption，专门处理 web 图文数据噪声。

### BLIP 的三个预训练目标分别学什么？

回答思路：分别讲 ITC、ITM、LM，区分全局对齐、细粒度匹配和生成能力。

回答模板：

ITC 是图文对比学习，建立全局图文表示对齐；ITM 将图文特征融合后判断是否匹配，学习更细粒度的跨模态关系；LM 在图像条件下自回归生成 caption，学习视觉到语言的生成能力。三者结合使 BLIP 同时具备理解和生成能力。

### CapFilt 是怎么工作的？为什么有用？

回答思路：按 captioner 生成、filter 过滤、继续训练三步回答。

回答模板：

CapFilt 用 captioner 为图像生成合成 caption，再用 filter 判断原始文本和合成 caption 是否与图像匹配，只保留高质量图文对，最后用这些 bootstrapped 数据继续预训练。它把模型的生成能力用于扩充数据，把匹配能力用于清洗噪声，从而降低 web caption 错配对训练的影响。
